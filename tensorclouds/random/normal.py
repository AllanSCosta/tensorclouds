...
class NormalDistribution():
    ...
    def sample(
        ...
        mask = None #TODO: remove this redundant
    ) -> TensorCloud:
        ...
        if mask_features is not None:
            mask = mask_features
            # irreps = irreps * mask_features #TODO: FIX IT TO HAVE MASK 
        if mask_coord is not None:
            mask = mask_coord #TODO: fix that.
            coords = coords * mask_coord[..., None]
        print(f'in normal, irreps: {irreps} and coords: {irreps}')
        return TensorCloud(
            irreps_array=irreps,
